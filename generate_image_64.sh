#!/bin/bash -e
# ami-0a59bb63 seems to be a good 64bit 5.4 ami

#ami-08f41161

export IMAGE_NAME=refactoredcentos5x86_64
...
bundleVolume() {
    ...
    mkdir -p /mnt/tmp
    if [ -z "$KERNEL_ID" ]; then
      ec2-bundle-vol -v /mnt/ec2-fs -d /mnt/tmp -p $IMAGE_NAME -k $EC2_PRIVATE_KEY -c $EC2_CERT -u $AWS_ACCOUNT_NUMBER --fstab /mnt/ec2-fs/etc/fstab -r x86_64
    else 
      ec2-bundle-vol -v /mnt/ec2-fs -d /mnt/tmp -p $IMAGE_NAME -k $EC2_PRIVATE_KEY -c $EC2_CERT -u $AWS_ACCOUNT_NUMBER --fstab /mnt/ec2-fs/etc/fstab -r x86_64 --kernel $KERNEL_ID
    fi
